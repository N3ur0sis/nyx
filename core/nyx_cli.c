...
static const char* ansi_colors[] = {
    [NYX_CLI_COLOR_NONE] = "\033[0m",
    [NYX_CLI_COLOR_RED] = "\033[1;31m",
    [NYX_CLI_COLOR_GREEN] = "\033[1;32m",
    [NYX_CLI_COLOR_YELLOW] = "\033[1;33m",
    [NYX_CLI_COLOR_BLUE] = "\033[1;34m",
    [NYX_CLI_COLOR_MAGENTA] = "\033[1;35m",
    [NYX_CLI_COLOR_CYAN] = "\033[1;36m",
    [NYX_CLI_COLOR_WHITE] = "\033[1;37m"
};

/**
 * Helper function to create a long_options array for getopt_long
 */
static struct option* create_long_options(const nyx_cli_opt_def_t *opt_defs, 
                                         size_t opt_count,
                                         char **optstring) {
    ...
    for (size_t i = 0; i < opt_count; i++) {
        if (opt_defs[i].flags & NYX_CLI_FLAG_REQUIRED) {
            ...
            if (idx >= 0 && !result->options[idx].present) {
                nyx_log(NYX_LOG_ERROR, "Required option -%c%s%s is missing", 
                        opt_defs[i].short_opt,
                        opt_defs[i].long_opt ? "/--" : "",
...
void nyx_cli_print_banner(const nyx_cli_banner_config_t *config) {
    ...
    if (config->style == NYX_CLI_BANNER_ASCII_ART) {
        ...
        printf("%s          NYX OFFENSIVE SUITE MODULE - %s%s\n", 
               secondary_color,
               config->tool_name ? config->tool_name : "Unknown Tool",
               reset);
        if (config->author) {
            printf("%s          Author: %s%s\n", 
                  secondary_color, 
