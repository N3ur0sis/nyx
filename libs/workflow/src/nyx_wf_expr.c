...
{
    if (!src) return nyx_json_null();
    switch (nyx_json_type(src)) {
        case NYX_JSON_NULL:   return nyx_json_null();
        case NYX_JSON_BOOL:   return nyx_json_bool(nyx_json_get_bool(src));
        case NYX_JSON_INT:    return nyx_json_int(nyx_json_get_int(src));
        case NYX_JSON_DOUBLE: return nyx_json_real(nyx_json_get_real(src));
        case NYX_JSON_STRING: return nyx_json_string(nyx_json_get_string(src));
        case NYX_JSON_ARRAY: {
...
{
    if (!node) return str_dup("");
    switch (nyx_json_type(node)) {
        case NYX_JSON_STRING: return str_dup(nyx_json_get_string(node));
        case NYX_JSON_INT: {
            char buf[32];
...
static const nyx_json_t *resolve_path(const nyx_json_t *root, const char *path)
{
    ...
    while (*p && cur) {
        ...
        if (*p == '[') {
            ...
            if (*end != ']') return NULL;
            p = end + 1;
            if (nyx_json_type(cur) != NYX_JSON_ARRAY) return NULL;
        ...
        if (nyx_json_type(cur) != NYX_JSON_OBJECT) return NULL;
...
static int compare_values(const nyx_json_t *a, const char *op,
                          const char *val_str)
{
    if (!a) return 0;
    ...
    if (strcmp(val_str, "true") == 0 || strcmp(val_str, "false") == 0) {
        ...
        if (nyx_json_type(a) != NYX_JSON_BOOL) return 0;
        int aval = nyx_json_get_bool(a);
        if (strcmp(op, "==") == 0) return aval == bval;
        if (strcmp(op, "!=") == 0) return aval != bval;
        return 0;
    }
    ...
    /* Numeric comparison */
    double av = 0, bv = 0;
    if (nyx_json_type(a) == NYX_JSON_INT) av = (double)nyx_json_get_int(a);
    else if (nyx_json_type(a) == NYX_JSON_DOUBLE) av = nyx_json_get_real(a);
    else return 0;

    char *end;
    bv = strtod(val_str, &end);
    if (*end != '\0' && !isspace((unsigned char)*end)) return 0;

    if (strcmp(op, "==") == 0) return av == bv;
    if (strcmp(op, "!=") == 0) return av != bv;
    if (strcmp(op, ">") == 0)  return av > bv;
    if (strcmp(op, "<") == 0)  return av < bv;
    if (strcmp(op, ">=") == 0) return av >= bv;
...
static nyx_json_t *pipe_filter(const nyx_json_t *arr, const char **p)
{
    ...
    char val[256];
    if (vlen >= sizeof(val)) return NULL;
    memcpy(val, start, vlen);
    val[vlen] = '\0';

    if (nyx_json_type(arr) != NYX_JSON_ARRAY) return NULL;

    nyx_json_t *result = nyx_json_array();
    size_t len = nyx_json_length(arr);
    for (size_t i = 0; i < len; i++) {
        const nyx_json_t *elem = nyx_json_at(arr, i);
        if (nyx_json_type(elem) != NYX_JSON_OBJECT) continue;
 ...
 */
static const nyx_json_t *resolve_root(const char *root_id,
                                       const nyx_wf_ctx_t *ctx,
                                       const nyx_json_t *each_item)
{
    if (!root_id || !ctx) return NULL;
                              ...
                              const nyx_json_t *each_item)
{
    ...
    if (rlen == 0) return nyx_json_null();

    char root_id[128];
    if (rlen >= sizeof(root_id)) return nyx_json_null();
    memcpy(root_id, root_start, rlen);
    root_id[rlen] = '\0';

    const nyx_json_t *root = resolve_root(root_id, ctx, each_item);
    if (!root) return nyx_json_null();
    ...
    if (!resolved) return nyx_json_null();
                           ...
                           const nyx_json_t *each_item)
{
    ...
    char root_id[128];
    if (rlen >= sizeof(root_id)) return 0;
    memcpy(root_id, root_start, rlen);
    root_id[rlen] = '\0';
    ...
    if (p >= end || (!*p) || *p == '}') {
        switch (nyx_json_type(resolved)) {
            case NYX_JSON_BOOL:   return nyx_json_get_bool(resolved);
            case NYX_JSON_INT:    return nyx_json_get_int(resolved) != 0;
            case NYX_JSON_DOUBLE: return nyx_json_get_real(resolved) != 0.0;
            case NYX_JSON_STRING: return nyx_json_get_string(resolved)[0] != '\0';
            case NYX_JSON_NULL:   return 0;
            case NYX_JSON_ARRAY:  return nyx_json_length(resolved) > 0;
            case NYX_JSON_OBJECT: return 1;
        }
        return 0;
    }
                                  ...
                                  const nyx_json_t *each_item)
{
    ...
    if (!strchr(str, '$')) return str_dup(str);
    ...
    while (*p) {
        if (p[0] == '$' && p[1] == '{') {
            ...
            if (!expr_str) { free(out); return NULL; }
            ...
            if (val_str) {
                ...
                while (len + vlen + 1 >= cap) {
                    cap *= 2;
                    char *tmp = realloc(out, cap);
                    if (!tmp) { free(val_str); free(out); return NULL; }
                    out = tmp;
                }
    ...
    *count = 0;
    ...
    while (*p) {
        if (p[0] == '$' && p[1] == '{') {
            ...
            char id[128];
            if (klen >= sizeof(id)) continue;
            memcpy(id, start, klen);
            id[klen] = '\0';

            if (is_builtin_root(id)) continue;

            /* Deduplicate */
            int found = 0;
            for (size_t i = 0; i < n; i++) {
                if (strcmp(arr[i], id) == 0) { found = 1; break; }
            }
            if (!found) {
                if (n >= cap) {
                    cap *= 2;
