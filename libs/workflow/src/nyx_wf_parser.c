 ...
 */
static void collect_refs_from_string(const char *str, nyx_wf_step_t *step,
                                     const nyx_wf_step_t *all_steps, size_t step_count)
{
    if (!str)
        return;
    ...
    char **ids = NULL;
    size_t id_count = 0;
    if (nyx_wf_expr_scan_refs(str, &ids, &id_count) != 0)
        return;
    ...
    for (size_t i = 0; i < id_count; i++) {
        ...
        if (!dup) {
            size_t *tmp = realloc(step->deps, (step->dep_count + 1) * sizeof(size_t));
            if (tmp) {
                step->deps = tmp;
 ...
 */
static void scan_json_for_refs(const nyx_json_t *node, nyx_wf_step_t *step,
                               const nyx_wf_step_t *all_steps, size_t step_count)
{
    if (!node)
        return;
...
{
    if (!root || nyx_json_type(root) != NYX_JSON_OBJECT)
        return NULL;
    ...
    for (size_t i = 0; i < sc; i++) {
        ...
        if (!step_json || nyx_json_type(step_json) != NYX_JSON_OBJECT)
            continue;
