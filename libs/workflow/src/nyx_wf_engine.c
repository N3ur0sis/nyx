 ...
 */
static invoke_result_t invoke_tool(const char *tool_name,
                                   const nyx_json_t *params)
{
    ...
    nyx_output_ctx_t *out = nyx_output_init(tool->name, tool->module,
                                             tool->version);
    if (!out) return ir;
...
static int tool_reported_error(const nyx_json_t *envelope)
{
    ...
    if (!st || nyx_json_type(st) != NYX_JSON_STRING) return 0;
...
static nyx_json_t *resolve_params(const nyx_json_t *params,
                                   const nyx_wf_ctx_t *ctx,
                                   const nyx_json_t *each_item)
{
    if (!params) return nyx_json_object();

    char *s = nyx_json_serialize(params, 0);
    if (!s) return nyx_json_object();
            ...
            if (sv && strchr(sv, '$')) {
                ...
                if (check[0] == '$' && check[1] == '{') {
                    ...
                    if (close) {
                        ...
                        while (*after == ' ') after++;
                        if (!*after) {
                            nyx_json_t *val = nyx_wf_expr_eval(sv, ctx,
                                                                each_item);
                ...
                nyx_json_set(resolved, key,
                             nyx_json_string(rs ? rs : ""));
                free(rs);
            } else {
                nyx_json_set(resolved, key,
                             nyx_json_string(sv ? sv : ""));
            }
        } else {
...
static int execute_step(nyx_wf_ctx_t *ctx, size_t step_idx)
{
    ...
    nyx_log(NYX_LOG_INFO, "[%s] Starting step '%s' (tool: %s)",
            wf_id, step->id, step->tool);

    if (step->when) {
        if (!nyx_wf_expr_eval_bool(step->when, ctx, NULL)) {
            nyx_log(NYX_LOG_INFO, "[%s] Step '%s' skipped (when: false)",
                    wf_id, step->id);
    ...
    /* Verify tool exists before resolving params */
    {
        ...
        if (!t) {
            nyx_log(NYX_LOG_ERROR, "[%s] Step '%s': tool '%s' not registered",
                    wf_id, step->id, step->tool);
    ...
    if (step->for_each) {
        ...
        if (!items || nyx_json_type(items) != NYX_JSON_ARRAY) {
            nyx_log(NYX_LOG_WARN, "[%s] Step '%s' for_each resolved to "
                    "non-array, skipping", wf_id, step->id);
            nyx_json_free(items);
            ctx->status[step_idx] = NYX_WF_STATUS_SKIPPED;
            return NYX_WF_SUCCESS;
        }
        ...
        nyx_log(NYX_LOG_INFO, "[%s] Step '%s' expanding for_each "
                "(%zu iterations)", wf_id, step->id, count);
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            nyx_json_t *resolved = resolve_params(step->params, ctx,
                                                   each_item);
            ...
            if (ir.envelope) {
                const nyx_json_t *results = nyx_json_get(ir.envelope,
                                                          "results");
                if (results) {
                    char *rs = nyx_json_serialize(results, 0);
                    nyx_json_t *rc = rs ? nyx_json_parse(rs)
                                        : nyx_json_object();
                    free(rs);
                ...
                if (st && nyx_json_type(st) == NYX_JSON_STRING)
                    nyx_json_set(iter, "status",
                                 nyx_json_string(nyx_json_get_string(st)));
        ...
        nyx_json_set(step_result, "status",
                     nyx_json_string(any_error ? "error" : "success"));
        nyx_json_set(step_result, "iterations", iterations);

        nyx_json_t *merged = nyx_json_object();
        nyx_json_set(merged, "iteration_count",
                     nyx_json_int((long)count));
        nyx_json_set(step_result, "results", merged);

        ctx->results[step_idx] = step_result;
        ctx->status[step_idx] = any_error ? NYX_WF_STATUS_ERROR
                                          : NYX_WF_STATUS_DONE;
        nyx_term_clear_status();
        nyx_json_free(items);

        if (any_error)
            nyx_log(NYX_LOG_ERROR, "[%s] Step '%s' completed with errors "
                    "(%zu iterations)", wf_id, step->id, count);
        else
            nyx_log(NYX_LOG_SUCCESS, "[%s] Step '%s' completed "
                    "(%zu iterations)", wf_id, step->id, count);
    ...
    if (!ir.envelope) {
        nyx_log(NYX_LOG_ERROR, "[%s] Step '%s' failed (no output, rc %d)",
                wf_id, step->id, ir.rc);
        ctx->status[step_idx] = NYX_WF_STATUS_ERROR;
        return NYX_WF_ERR_EXEC;
    }
    ...
    if (tool_reported_error(ir.envelope) || ir.rc != 0) {
        ...
        nyx_log(NYX_LOG_ERROR, "[%s] Step '%s' error%s%s",
                wf_id, step->id, emsg[0] ? ": " : "", emsg);
...
static int preflight_privilege_check(const nyx_wf_ctx_t *ctx)
{
    ...
    for (size_t i = 0; i < wf->step_count; i++) {
        ...
        if (t->required_priv != NYX_PRIV_NONE &&
            !nyx_priv_check((nyx_priv_t)t->required_priv)) {
            if (!printed_header) {
                nyx_log(NYX_LOG_WARN,
                        "Workflow requires elevated privileges for:");
                printed_header = 1;
            }
            nyx_log(NYX_LOG_WARN, "  - %s (step: %s) [%s]",
                    t->name, wf->steps[i].id,
    ...
    if (needs_priv) {
        ...
        nyx_log(NYX_LOG_WARN,
                "Continuing anyway -- privileged steps will fail.");
    }
...
{
    if (!ctx || !ctx->workflow) return NYX_WF_ERR_PARAM;
...
{
    if (!ctx || !ctx->workflow) return nyx_json_object();
    ...
    for (size_t i = 0; i < ctx->workflow->step_count; i++) {
        ...
        switch (ctx->status[i]) {
            case NYX_WF_STATUS_DONE:    status_str = "success"; break;
            case NYX_WF_STATUS_SKIPPED: status_str = "skipped"; break;
            case NYX_WF_STATUS_ERROR:   status_str = "error";   break;
            default:                    status_str = "pending";  break;
        }
        nyx_json_set(step_out, "status", nyx_json_string(status_str));

        if (ctx->results[i]) {
            const nyx_json_t *results = nyx_json_get(ctx->results[i],
                                                      "results");
            if (results) {
                char *s = nyx_json_serialize(results, 0);
                nyx_json_t *clone = s ? nyx_json_parse(s)
                                      : nyx_json_object();
                free(s);
                nyx_json_set(step_out, "results", clone);
            }

            const nyx_json_t *iters = nyx_json_get(ctx->results[i],
                                                     "iterations");
            if (iters) {
            ...
            const nyx_json_t *error = nyx_json_get(ctx->results[i],
                                                     "error");
            if (error) {
