...
int nyx_iface_is_valid(const char *iface)
{
    ...
    if (len >= IFNAMSIZ) {
        nyx_log(NYX_LOG_ERROR, "Interface validation failed: name too long (%zu >= %d)", len,
                IFNAMSIZ);
        return 0;
    }
    ...
    for (int i = 0; i < MAC_ADDRESS_LENGTH; i++) {
        if ((i + 1) % 3 == 0) {
            ...
            if (!isxdigit((unsigned char)mac[i])) {
                nyx_log(NYX_LOG_ERROR, "MAC validation failed: invalid hex digit at position %d",
                        i);
                return 0;
            }
        }
...
int nyx_iface_is_up(const char *iface)
{
    ...
    if (sock < 0) {
        nyx_log(NYX_LOG_ERROR, "Failed to create socket for interface check: %s", strerror(errno));
        return 0;
    }
...
int nyx_iface_get_mac_by_ip(const char *iface, const char *ip_addr, char *mac_buffer, size_t len)
{
    ...
    if (ioctl(sock, SIOCGARP, &arpreq) < 0) {
        int err = errno;
        close(sock);
        nyx_log(NYX_LOG_ERROR, "ARP lookup failed for %s on %s: %s", ip_addr, iface, strerror(err));
    ...
    snprintf(mac_buffer, len, "%02x:%02x:%02x:%02x:%02x:%02x", mac_data[0], mac_data[1],
...
int nyx_iface_print_details(void)
{
    ...
    printf("------------------\n");
    printf("%-12s %-17s %-15s %-15s %s\n", "Interface", "MAC Address", "IPv4 Address", "Netmask",
           "Status");
    printf("%-12s %-17s %-15s %-15s %s\n", "---------", "-----------", "-----------", "-------",
           "------");
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        printf("%-12s %-17s %-15s %-15s %s\n", interfaces[i], mac, ip[0] ? ip : "N/A",
               netmask[0] ? netmask : "N/A", is_up ? "UP" : "DOWN");
    }
...
int nyx_iface_set_status(const char *iface, int up)
{
    ...
    if (ioctl(sock, SIOCGIFFLAGS, &ifr) < 0) {
        int err = errno;
        close(sock);
        nyx_log(NYX_LOG_ERROR, "Failed to get interface flags for %s: %s", iface, strerror(err));
        return map_error(err, NYX_IFACE_ERR_IO);
    }
    ...
    if (ioctl(sock, SIOCSIFFLAGS, &ifr) < 0) {
        int err = errno;
        close(sock);
        nyx_log(NYX_LOG_ERROR, "Failed to set interface %s %s: %s", iface, up ? "up" : "down",
                strerror(err));
        return map_error(err, NYX_IFACE_ERR_IO);
    }
...
int nyx_iface_add_ipv4(const char *iface, const char *ip_addr, int prefix_len)
{
    ...
    if (ioctl(sock, SIOCSIFADDR, &ifr) < 0) {
        int err = errno;
        close(sock);
        nyx_log(NYX_LOG_ERROR, "Failed to set IP address for %s: %s", iface, strerror(err));
        return map_error(err, NYX_IFACE_ERR_IO);
    }
    ...
    if (ioctl(sock, SIOCSIFNETMASK, &ifr) < 0) {
        int err = errno;
        close(sock);
        nyx_log(NYX_LOG_ERROR, "Failed to set netmask for %s: %s", iface, strerror(err));
        return map_error(err, NYX_IFACE_ERR_IO);
    }
