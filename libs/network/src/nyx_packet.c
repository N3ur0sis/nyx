...
int nyx_packet_create_icmp_echo(void *buffer, size_t buffer_len, uint16_t id, uint16_t seq,
...
int nyx_packet_create_arp(void *buffer, size_t buffer_len, const nyx_arp_params_t *params)
{
    if (!buffer ||
        buffer_len <
            sizeof(struct arphdr) + 2 * ARP_HARDWARE_ADDR_LEN + 2 * ARP_PROTOCOL_ADDR_LEN ||
        !params) {
        NYX_ERROR_SET(NYX_DOMAIN_CORE, NYX_ERR_PARAM,
...
int nyx_packet_parse_icmp(const void *packet, size_t packet_len, nyx_icmp_params_t *params)
{
...
int nyx_packet_parse_arp(const void *packet, size_t packet_len, nyx_arp_params_t *params)
{
    ...
    if (ntohs(arp->ar_hrd) != ARP_HARDWARE_TYPE || ntohs(arp->ar_pro) != ETH_P_IP ||
        arp->ar_hln != ARP_HARDWARE_ADDR_LEN || arp->ar_pln != ARP_PROTOCOL_ADDR_LEN) {
        NYX_ERROR_SET(NYX_DOMAIN_CORE, NYX_ERR_GENERIC, "Invalid ARP packet format");
...
uint16_t nyx_packet_tcp_checksum(uint32_t src_ip, uint32_t dst_ip, const void *tcp_seg,
                                 size_t tcp_len)
{
...
int nyx_packet_create_ip_tcp_syn(void *buffer, size_t buffer_len, const nyx_tcp_params_t *params)
{
    const size_t total_len = IP_HEADER_LEN + TCP_HEADER_LEN;

    if (!buffer || !params) {
        NYX_ERROR_SET(NYX_DOMAIN_CORE, NYX_ERR_PARAM, "NULL buffer or params for TCP SYN packet");
        return NYX_PACKET_ERR_PARAM;
    }
    ...
    tcp->check = nyx_packet_tcp_checksum(params->src_ip, params->dst_ip, tcp, TCP_HEADER_LEN);

    return (int)total_len;
}

int nyx_packet_parse_tcp(const void *packet, size_t packet_len, nyx_tcp_parsed_t *out)
{
    if (!packet || !out) {
        NYX_ERROR_SET(NYX_DOMAIN_CORE, NYX_ERR_PARAM,
    ...
    const struct tcphdr *tcp = (const struct tcphdr *)((const uint8_t *)packet + ip_hdr_len);
