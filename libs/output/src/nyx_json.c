...
nyx_json_t *nyx_json_object(void)  { return node_new(NYX_JSON_OBJECT); }
nyx_json_t *nyx_json_array(void)   { return node_new(NYX_JSON_ARRAY); }
nyx_json_t *nyx_json_null(void)    { return node_new(NYX_JSON_NULL); }
...
{
    if (!node) return 0.0;
    if (node->type == NYX_JSON_DOUBLE) return node->v.d;
    if (node->type == NYX_JSON_INT)    return (double)node->v.i;
    return 0.0;
}
...
static void sbuf_grow(sbuf_t *sb, size_t need)
{
    ...
    if (!tmp) { free(sb->buf); sb->buf = NULL; return; }
    sb->buf = tmp;
    sb->cap = new_cap;
}
...
static void serialize_node(sbuf_t *sb, const nyx_json_t *node,
                           int depth, int indent)
{
    if (!node) { sbuf_puts(sb, "null"); return; }
    ...
    case NYX_JSON_ARRAY: {
        sbuf_putc(sb, '[');
        nyx_json_t *c = node->child;
        if (c && indent > 0) sbuf_putc(sb, '\n');
        int first = 1;
        for (; c; c = c->next) {
            if (!first) {
                sbuf_putc(sb, ',');
                if (indent > 0) sbuf_putc(sb, '\n');
            }
            first = 0;
...
static char *parse_string_raw(parser_t *p)
{
    ...
    while (p->src[p->pos] && p->src[p->pos] != '"') {
        if (p->src[p->pos] == '\\') {
            p->pos++;
            switch (p->src[p->pos]) {
            case '"':  sbuf_putc(&sb, '"');  p->pos++; break;
            case '\\': sbuf_putc(&sb, '\\'); p->pos++; break;
            case '/':  sbuf_putc(&sb, '/');  p->pos++; break;
            case 'b':  sbuf_putc(&sb, '\b'); p->pos++; break;
            case 'f':  sbuf_putc(&sb, '\f'); p->pos++; break;
            case 'n':  sbuf_putc(&sb, '\n'); p->pos++; break;
            case 'r':  sbuf_putc(&sb, '\r'); p->pos++; break;
            case 't':  sbuf_putc(&sb, '\t'); p->pos++; break;
            case 'u': {
                p->pos++;
                char hex[5] = {0};
    ...
    if (p->src[p->pos] == '"') p->pos++;  /* consume closing quote */
    return sb.buf;
}
...
static nyx_json_t *parse_number(parser_t *p)
{
    ...
    if (end == start || errno == ERANGE) return NULL;
    p->pos += (size_t)(end - start);
    nyx_json_t *n = node_new(NYX_JSON_INT);
    if (n) n->v.i = lval;
    return n;
...
static nyx_json_t *parse_array(parser_t *p)
{
    ...
    if (peek(p) == ']') { advance(p); return arr; }

    for (;;) {
        nyx_json_t *val = parse_value(p);
        if (!val) { nyx_json_free(arr); return NULL; }
        nyx_json_append(arr, val);
        if (!match(p, ',')) break;
    }

    if (!match(p, ']')) { nyx_json_free(arr); return NULL; }
    return arr;
}
...
static nyx_json_t *parse_object(parser_t *p)
{
    ...
    if (peek(p) == '}') { advance(p); return obj; }

    for (;;) {
        char *key = parse_string_raw(p);
        if (!key) { nyx_json_free(obj); return NULL; }

        if (!match(p, ':')) { free(key); nyx_json_free(obj); return NULL; }

        nyx_json_t *val = parse_value(p);
        if (!val) { free(key); nyx_json_free(obj); return NULL; }
    ...
    if (!match(p, '}')) { nyx_json_free(obj); return NULL; }
    return obj;
}
...
static nyx_json_t *parse_value(parser_t *p)
{
    ...
    if (match_literal(p, "true"))  return nyx_json_bool(1);
    if (match_literal(p, "false")) return nyx_json_bool(0);
    if (match_literal(p, "null"))  return nyx_json_null();
