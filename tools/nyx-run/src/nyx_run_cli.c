...
static const nyx_cli_opt_def_t cli_options[] = {
    {'V', "var", "KEY=VAL", "Override a workflow variable (repeatable)", NYX_CLI_ARG_REQUIRED,
     NYX_CLI_FLAG_OPTIONAL},
    {'b', "bin-dir", "PATH", "Path to NYX tool binaries", NYX_CLI_ARG_REQUIRED,
     NYX_CLI_FLAG_OPTIONAL},
    {'J', "json", NULL, "Output results as JSON to stdout", NYX_CLI_ARG_NONE,
     NYX_CLI_FLAG_OPTIONAL},
    {'S', "session", "ID", "Use specific session ID", NYX_CLI_ARG_REQUIRED, NYX_CLI_FLAG_OPTIONAL},
    {'d', "debug", NULL, "Enable verbose execution logging", NYX_CLI_ARG_NONE,
     NYX_CLI_FLAG_OPTIONAL},
 ...
 */
static void apply_var_overrides(nyx_workflow_t *wf, int argc, char *argv[])
{
    for (int i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "--var") == 0 || strcmp(argv[i], "-V") == 0) && i + 1 < argc) {
            i++;
...
int main(int argc, char *argv[])
{
    ...
    if (nyx_cli_has_option(cli, 'h')) {
        nyx_cli_print_usage("nyx-run <workflow.json> [OPTIONS]", cli_options, cli_option_count,
    ...
    nyx_output_ctx_t *out =
        nyx_output_from_cli("nyx-run", NYX_MODULE, NYX_VERSION, json_mode, session_id);
    ...
    /* Banner */
    if (banner_enabled) {
        nyx_cli_banner_config_t banner = {.module = NYX_MODULE,
                                          .tool_name = "nyx-run",
                                          .version = NYX_VERSION,
                                          .author = "Neur0sis",
    ...
    if (!wf_path || !wf_path[0]) {
        if (nyx_output_is_json_mode(out)) {
            nyx_output_emit_error_msg(out,
                                      "No workflow file specified. Usage: nyx-run <workflow.json>");
            out = NULL;
        } else {
    ...
    /* Validate */
    nyx_log(NYX_LOG_INFO, "Validating workflow '%s' (%zu steps)...", wf->id ? wf->id : wf_path,
            wf->step_count);

    int rc = nyx_wf_validate(wf);
    if (rc != NYX_WF_SUCCESS) {
        const char *msg = (rc == NYX_WF_ERR_CYCLE) ? "Workflow contains a dependency cycle"
    ...
    /* Display execution order */
    {
        ...
        if (nyx_wf_topo_sort(wf, &order, &order_len) == NYX_WF_SUCCESS) {
            for (size_t i = 0; i < order_len; i++)
                nyx_log(NYX_LOG_INFO, "  %zu. %s (%s)", i + 1, wf->steps[order[i]].id,
    ...
    /* Record workflow config in output envelope */
    {
        ...
        if (wf->id)
            nyx_json_set(config, "workflow_id", nyx_json_string(wf->id));
        if (wf->name)
            nyx_json_set(config, "workflow_name", nyx_json_string(wf->name));
    ...
    if (wf->id)
        nyx_json_set(wf_results, "id", nyx_json_string(wf->id));
    if (wf->name)
        nyx_json_set(wf_results, "name", nyx_json_string(wf->name));
