...
static const nyx_repl_flag_t run_flags[] = {
    {"--var", NYX_COMPL_NONE},  {"-V", NYX_COMPL_NONE}, {"-J", NYX_COMPL_NONE},
    {"--json", NYX_COMPL_NONE}, {"-S", NYX_COMPL_NONE}, {"--session", NYX_COMPL_NONE},
};
...
static const nyx_repl_cmd_t master_commands[] = {
    {.name = "run",
     ...
     .help = "  Loads a JSON workflow and executes it through the workflow engine.\n"
    ...
    {.name = "tools",
     ...
     .help = "  Shows all tools registered in the current binary.\n"
...
static void enter_tool_context(nyx_ctx_t *ctx, const char *tool_name)
{
    ...
    if (tool->required_priv != NYX_PRIV_NONE && !nyx_priv_check((nyx_priv_t)tool->required_priv)) {
        nyx_log(NYX_LOG_WARN,
                "This tool requires %s. "
    ...
    if (tool->cmds && tool->cmd_count > 0) {
        nyx_repl_add_cmds(sub, (const nyx_repl_cmd_t *)tool->cmds, tool->cmd_count);
    }
...
static int cmd_run(int argc, char **argv, void *data)
{
    ...
    nyx_output_ctx_t *out =
        nyx_output_from_cli("nyx-run", "workflow", NYX_VERSION, json_mode, session_id);

    nyx_workflow_t *wf = nyx_wf_load_file(wf_path);
    if (!wf) {
        nyx_log(NYX_LOG_ERROR, "Failed to load workflow: %s", wf_path);
        if (json_mode)
            nyx_output_emit_error_msg(out, "Failed to load workflow");
        nyx_output_free(out);
        return 1;
    }
    ...
    if (nyx_wf_validate(wf) != NYX_WF_SUCCESS) {
        ...
        if (json_mode)
            nyx_output_emit_error_msg(out, "Workflow validation failed");
        nyx_wf_free(wf);
        nyx_output_free(out);
        return 1;
    }

    nyx_log(NYX_LOG_INFO, "Executing workflow '%s' (%zu steps)...", wf->id ? wf->id : wf_path,
            wf->step_count);
    ...
    if (tool) {
        ...
        if (tool->cmds && tool->cmd_count > 0) {
            for (size_t i = 0; i < tool->cmd_count; i++) {
                const nyx_repl_cmd_t *c = (const nyx_repl_cmd_t *)&tool->cmds[i];
                if (strcmp(argv[1], c->name) == 0)
                    return c->handler(argc - 1, argv + 1, NULL);
            }
            fprintf(stderr,
                    "nyx: tool '%s' has no command '%s'. "
...
{
    nyx_cli_banner_config_t banner = {.module = "framework",
                                      .tool_name = "nyx",
                                      .version = NYX_VERSION,
                                      .author = "Neur0sis",
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 2) {
        ...
        if (strcmp(cmd, "help") == 0 || strcmp(cmd, "--help") == 0 || strcmp(cmd, "-h") == 0) {
            print_banner();
            printf(COLOR_CYAN "Usage:" COLOR_RESET " nyx [command]\n\n"
    ...
    nyx_repl_add_cmds(repl, master_commands, sizeof(master_commands) / sizeof(master_commands[0]));
