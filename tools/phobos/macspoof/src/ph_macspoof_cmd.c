...
int ph_macspoof_cmd_invoke(const nyx_json_t *params, nyx_output_ctx_t *out)
{
    ...
    if (iface)
        nyx_json_set(jcfg, "interface", nyx_json_string(iface));
    ...
    if (!iface || !iface[0]) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_NO_IFACE, NYX_ERROR_SEV_ERROR,
                         "No interface specified", "Provide 'i' or 'interface' parameter");
        nyx_output_set_error_from_ctx(out);
    ...
    if (strcmp(op, "show") == 0) {
        ...
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_INVALID_MAC, NYX_ERROR_SEV_ERROR,
                         "Unknown operation", "Use: show, random, custom, restore, or list");
        nyx_json_free(results);
        nyx_output_set_error_from_ctx(out);
        return -1;
    }
...
const nyx_repl_cmd_t ph_macspoof_repl_cmds[] = {
    ...
    {.name = "random",
     ...
     .help = "  Generates a random locally-administered unicast MAC and applies it.\n"
    ...
    {.name = "restore",
     ...
     .help = "  Restores the original (permanent) MAC from backup.\n"
             "\n"
             "  Options:\n"
             "    -i, --interface <name>  Network interface [required]\n"
             "\n"
    ...
    {.name = "list",
     ...
     .help = "  Shows all network interfaces with their MAC, IP, netmask and state.\n"
