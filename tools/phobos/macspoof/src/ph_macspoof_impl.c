...
static int map_iface_error_to_macspoof(int err)
{
    ...
    case NYX_IFACE_ERR_NOTFOUND:
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_NO_IFACE, NYX_ERROR_SEV_ERROR,
                         "Interface not found",
                         "Verify the interface exists using 'ip link' or run with '-l' to list "
    ...
    case NYX_IFACE_ERR_BUSY:
        NYX_ERROR_SET_EX(
            NYX_DOMAIN_MACSPOOF, PH_ERR_BUSY, NYX_ERROR_SEV_WARNING,
...
{
    if (!iface) {
        NYX_ERROR_SET(NYX_DOMAIN_MACSPOOF, PH_ERR_NO_IFACE, "NULL interface name");
        return 0;
    }
...
static int atomic_file_write(const char *path, const char *content)
{
    ...
    if (ret >= (int)sizeof(tmp_path)) {
        NYX_ERROR_SET_EX(
            NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR, "Temporary path too long",
    ...
    if (fprintf(fp, "%s", content) < 0) {
        ...
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR, msg,
...
int ph_macspoof_save_original_mac(const char *iface, const char *mac)
{
    if (!nyx_iface_is_valid(iface)) {
        NYX_ERROR_SET_EX(
            NYX_DOMAIN_MACSPOOF, PH_ERR_NO_IFACE, NYX_ERROR_SEV_ERROR, "Invalid interface name",
    ...
    int ret = snprintf(backup_path, sizeof(backup_path), "%s/%s%s%s", backup_dir,
                       BACKUP_FILE_PREFIX, iface, BACKUP_FILE_SUFFIX);
    if (ret >= (int)sizeof(backup_path)) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR,
                         "Backup path too long for interface", "Use a shorter interface name");
        return PH_ERR_FILE_IO;
    }
    ...
    if (ret >= (int)sizeof(backup_content)) {
        return NYX_ERROR_SET(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, "Backup content too long");
    }
...
int ph_macspoof_load_original_mac(const char *iface, char *buffer, size_t len)
{
    ...
    if (ret >= (int)sizeof(backup_path)) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR,
                         "Backup path too long", "Use a shorter interface name");
        return PH_ERR_FILE_IO;
    }
    ...
    if (!fp) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_NOT_FOUND, NYX_ERROR_SEV_ERROR,
                         ...
                         "Either MAC was never changed or you're running as a different user than "
                         "when it was changed");
    ...
    if (!found_mac) {
        NYX_ERROR_SET_EX(
            NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR,
    ...
    if (!nyx_iface_is_valid_mac(saved_mac)) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_INVALID_MAC, NYX_ERROR_SEV_ERROR,
                         ...
                         "The backup file contains an invalid MAC address, try manually setting a "
                         "MAC address instead");
        return PH_ERR_INVALID_MAC;
    }
...
void ph_macspoof_delete_original_mac(const char *iface)
{
    ...
    if (unlink(backup_path) == 0) {
        ...
        nyx_log(NYX_LOG_ERROR, "Failed to delete backup for %s: %s", iface, strerror(errno));
    }
}
...
int ph_macspoof_change_mac(const char *iface, const char *mac)
{
    ...
    if (was_up) {
        if (nyx_iface_set_status(iface, 0) != NYX_IFACE_SUCCESS) {
            NYX_ERROR_SET_EX(
                NYX_DOMAIN_MACSPOOF, PH_ERR_IOCTL, NYX_ERROR_SEV_ERROR,
    ...
    if (ret != 0) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_IOCTL, NYX_ERROR_SEV_ERROR,
                         ...
                         "This may be due to hardware restrictions, driver limitations, or "
    ...
    if (was_up) {
        ...
        if (nyx_iface_set_status(iface, 1) != NYX_IFACE_SUCCESS) {
            ...
            NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_IOCTL, NYX_ERROR_SEV_ERROR,
                             ...
                             "Interface may be left in DOWN state; bring it up manually: ip link "
                             "set <iface> up");
            return PH_ERR_IOCTL;
        }
...
int ph_macspoof_restore_mac(const char *iface)
{
    ...
    if (load_result != PH_SUCCESS) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, load_result, NYX_ERROR_SEV_ERROR,
                         ...
                         "Try running with the same privileges used when changing the MAC, or "
...
int ph_macspoof_generate_random_mac(char *buffer, size_t len)
{
    ...
    if (!urand) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR,
                         "Failed to open /dev/urandom", "Ensure /dev/urandom is accessible");
        return PH_ERR_FILE_IO;
    }
    if (fread(entropy, 1, sizeof(entropy), urand) != sizeof(entropy)) {
        fclose(urand);
        NYX_ERROR_SET_EX(NYX_DOMAIN_MACSPOOF, PH_ERR_FILE_IO, NYX_ERROR_SEV_ERROR,
                         "Failed to read from /dev/urandom", "System entropy source unavailable");
        return PH_ERR_FILE_IO;
    }
    fclose(urand);
...
int ph_macspoof_random_mac(const char *iface)
{
    ...
    nyx_log(NYX_LOG_SUCCESS, "Successfully applied random MAC %s to interface %s", mac_addr, iface);
    return PH_SUCCESS;
}
...
int ph_macspoof_custom_mac(const char *iface, const char *mac)
{
    ...
    nyx_log(NYX_LOG_SUCCESS, "Successfully applied custom MAC %s to interface %s", mac, iface);
    return PH_SUCCESS;
}
