...
{
    if (!ip || !latency_ms) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_INVALID_PARAM, NYX_ERROR_SEV_ERROR,
                         "NULL parameter to ping_host", "Provide valid IP and latency pointer");
    ...
    *latency_ms = 0.0;
    ...
    int pkt_len = nyx_packet_create_icmp_echo(pkt, sizeof(pkt), id, seq, "NYX", 3);
    if (pkt_len < 0) {
    ...
    ssize_t sent = sendto(sockfd, pkt, (size_t)pkt_len, 0, (struct sockaddr *)&dest, sizeof(dest));
    if (sent < 0) {
        nyx_socket_close(sockfd);
        return PH_PINGSWEEP_ERR_SEND;
    }
    ...
    while (max_attempts-- > 0) {
        ...
        ssize_t n =
            recvfrom(sockfd, recv_buf, sizeof(recv_buf), 0, (struct sockaddr *)&from, &from_len);
        if (n < 0) {
            nyx_socket_close(sockfd);
        ...
        if (icmp->type == 0 && ntohs(icmp->un.echo.id) == id &&
 ...
 */
static int ping_host_with_socket(int sockfd, const char *ip, int timeout_ms, uint16_t echo_id,
                                 uint16_t seq, double *latency_ms)
{
    *latency_ms = 0.0;
    ...
    uint8_t pkt[ICMP_PAYLOAD_SIZE + 8];
    int pkt_len = nyx_packet_create_icmp_echo(pkt, sizeof(pkt), echo_id, seq, "NYX", 3);
    if (pkt_len < 0)
        return PH_PINGSWEEP_ERR_SEND;
    ...
    while (max_attempts-- > 0) {
        ...
        if (icmp->type == 0 && ntohs(icmp->un.echo.id) == echo_id &&
            ntohs(icmp->un.echo.sequence) == seq) {
            clock_gettime(CLOCK_MONOTONIC, &t_end);
            *latency_ms = timespec_diff_ms(&t_start, &t_end);
            return PH_PINGSWEEP_SUCCESS;
...
static void sweep_progress_update(sweep_progress_t *progress, const char *ip, int alive,
                                  double latency_ms)
{
    if (!progress || !ip)
        return;
    ...
    size_t alive_count =
        alive ? (atomic_fetch_add(&progress->alive, 1) + 1) : atomic_load(&progress->alive);
    ...
    if (nyx_term_is_interactive()) {
        nyx_term_statusf("[*] Pingsweep %zu/%zu hosts (alive: %zu) current=%s", completed,
                         progress->total, alive_count, progress->last_ip);
    } else if (completed == 1 || completed == progress->total || (completed % 32) == 0) {
        nyx_log(NYX_LOG_INFO, "Pingsweep progress: %zu/%zu hosts scanned (alive: %zu)", completed,
                progress->total, alive_count);
    }
...
static void *sweep_worker(void *arg)
{
    ...
    for (size_t i = 0; i < w->count; i++) {
        ...
        snprintf(w->hosts[w->start_idx + i].ip, PH_PINGSWEEP_MAX_IP_LEN, "%s", ip_str);

        double lat = 0.0;
        uint16_t seq = (uint16_t)((i + 1) & 0xFFFF);
        ret = ping_host_with_socket(sockfd, ip_str, w->timeout_ms, w->echo_id, seq, &lat);

        w->hosts[w->start_idx + i].alive = (ret == PH_PINGSWEEP_SUCCESS) ? 1 : 0;
        w->hosts[w->start_idx + i].latency_ms = lat;
        sweep_progress_update(w->progress, ip_str, w->hosts[w->start_idx + i].alive, lat);
    }
...
int ph_pingsweep_scan(const ph_pingsweep_config_t *config, ph_pingsweep_result_t **result)
{
    ...
    int timeout = config->timeout_ms > 0 ? config->timeout_ms : PH_PINGSWEEP_DEFAULT_TIMEOUT;
    int nthreads = config->threads > 0 ? config->threads : PH_PINGSWEEP_DEFAULT_THREADS;
    ...
    if (!res) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_MEMORY, NYX_ERROR_SEV_CRITICAL,
    ...
    if (!res->hosts) {
        free(res);
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_MEMORY, NYX_ERROR_SEV_CRITICAL,
                         "Failed to allocate host array", "System may be low on memory");
        return PH_PINGSWEEP_ERR_MEMORY;
    }
    res->total = cidr.num_hosts;

    nyx_log(NYX_LOG_INFO, "Scanning %u hosts on %s (%d threads, %d ms timeout)", cidr.num_hosts,
            config->cidr, nthreads, timeout);
    ...
    if (all_socket_failed) {
        free(res->hosts);
        free(res);
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_SOCKET, NYX_ERROR_SEV_ERROR,
    ...
    nyx_log(NYX_LOG_SUCCESS, "Scan complete: %zu/%zu hosts alive (%.1f ms)", res->alive_count,
            res->total, res->elapsed_ms);
...
void ph_pingsweep_print_result(const ph_pingsweep_result_t *result)
{
    ...
    for (size_t i = 0; i < result->total; i++) {
        if (!result->hosts[i].alive)
            continue;
        printf("%-18s %-8s %.2f ms\n", result->hosts[i].ip, "UP", result->hosts[i].latency_ms);
    }
