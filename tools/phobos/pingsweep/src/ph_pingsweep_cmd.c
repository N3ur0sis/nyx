...
int ph_pingsweep_cmd_invoke(const nyx_json_t *params, nyx_output_ctx_t *out)
{
    ...
    if (!cidr) cidr = json_str(params, "cidr");

    if (!cidr || !cidr[0]) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_CIDR,
                         NYX_ERROR_SEV_ERROR, "No CIDR target specified",
                         "Provide 'c' or 'cidr' parameter");
        nyx_output_set_error_from_ctx(out);
        return PH_PINGSWEEP_ERR_CIDR;
    }

    if (geteuid() != 0) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PINGSWEEP, PH_PINGSWEEP_ERR_PERMISSION,
                         NYX_ERROR_SEV_ERROR,
                         "Pingsweep requires root privileges",
                         "Run with sudo");
        nyx_output_set_error_from_ctx(out);
    ...
    if (iface) strncpy(cfg.iface, iface, sizeof(cfg.iface) - 1);

    cfg.timeout_ms = (int)json_int(params, "t",
...
static int repl_scan(int argc, char **argv, void *data)
{
    ...
    for (int i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--cidr") == 0)
            && i + 1 < argc)
