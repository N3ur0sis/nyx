...
static int run_oneshot(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "-J") == 0 || strcmp(argv[i], "--json") == 0)
            ...
            session_id = argv[++i];
        else if ((strcmp(argv[i], "-c") == 0 || strcmp(argv[i], "--cidr") == 0) && i + 1 < argc)
...
int main(int argc, char *argv[])
{
    ...
    if (nyx_priv_ensure(NYX_PRIV_NET_RAW, argc, argv) != 0) {
        fprintf(stderr,
                "pingsweep: requires root privileges (raw ICMP sockets).\n"
                "  Run with: sudo %s\n",
                argv[0]);
        return 1;
    }
    ...
    nyx_repl_add_cmds(repl, ph_pingsweep_repl_cmds, ph_pingsweep_repl_cmd_count);
    nyx_repl_set_welcome(repl, "Type 'scan -c <cidr>' to sweep, 'list' for interfaces, "
                               "'help' for commands.");
