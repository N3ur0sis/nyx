...
{
    (void)sig;
    if (g_arp_session) ph_arpspoof_stop(g_arp_session);
}
...
int ph_arpspoof_cmd_invoke(const nyx_json_t *params, nyx_output_ctx_t *out)
{
    ...
    if (!spoof) spoof = json_str(params, "spoof");
    ...
    if (geteuid() != 0) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_PERMISSION,
                         NYX_ERROR_SEV_ERROR,
                         "ARP spoofing requires root privileges",
                         "Run with sudo");
        nyx_output_set_error_from_ctx(out);
    ...
    cfg.bidirectional = json_bool(params, "b",
                            json_bool(params, "bidirectional", 0));
    cfg.interval = (int)json_int(params, "n",
                        json_int(params, "interval",
                                 PH_ARPSPOOF_DEFAULT_INTERVAL));
    ...
    double dur = (double)(t1.tv_sec - t0.tv_sec) +
...
static int repl_start(int argc, char **argv, void *data)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if ((strcmp(argv[i], "-s") == 0 || strcmp(argv[i], "--spoof") == 0)
                 && i + 1 < argc)
...
static const nyx_repl_flag_t start_flags[] = {
    ...
    { "-s",              NYX_COMPL_NONE  },
    { "--spoof",         NYX_COMPL_NONE  },
    { "-b",              NYX_COMPL_NONE  },
    { "--bidirectional", NYX_COMPL_NONE  },
    { "-n",              NYX_COMPL_NONE  },
    { "--interval",      NYX_COMPL_NONE  },
};
...
const nyx_repl_cmd_t ph_arpspoof_repl_cmds[] = {
    {
        ...
        .help =
            "  Sends crafted ARP replies to poison the target's ARP cache,\n"
