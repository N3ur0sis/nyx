 ...
 */
static int build_port_list(const ph_portscan_config_t *config,
                           uint16_t **ports, size_t *count)
{
    if (config->top_ports > 0) {
        int n = config->top_ports;
        if (n > TOP_PORTS_COUNT) n = TOP_PORTS_COUNT;

        uint16_t *arr = malloc(sizeof(uint16_t) * (size_t)n);
        if (!arr) return PH_PORTSCAN_ERR_MEMORY;
...
static ph_port_state_t connect_scan_port(const char *target, uint16_t port,
                                         int timeout_ms)
{
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0) return PH_PORT_FILTERED;
    ...
    if (so_err == 0) return PH_PORT_OPEN;
    if (so_err == ECONNREFUSED) return PH_PORT_CLOSED;
    return PH_PORT_FILTERED;
}
...
static ph_port_state_t syn_scan_port(uint32_t src_ip, uint32_t dst_ip,
                                     uint16_t port, int send_fd, int recv_fd,
                                     int timeout_ms)
{
    ...
    if (pkt_len < 0) return PH_PORT_FILTERED;
    ...
    if (sendto(send_fd, pkt, (size_t)pkt_len, 0,
    ...
    while (1) {
        ...
        ssize_t n = recvfrom(recv_fd, recv_buf, sizeof(recv_buf), 0,
                             (struct sockaddr *)&from, &from_len);
        if (n < 40) continue;

        if (from.sin_addr.s_addr != dst_ip) continue;
        ...
        if ((parsed.flags & (TH_SYN | TH_ACK)) == (TH_SYN | TH_ACK)) {
            ...
            if (rst_len > 0) {
                sendto(send_fd, rst_pkt, (size_t)rst_len, 0,
...
static void scan_progress_update(scan_progress_t *progress,
                                 uint16_t port,
                                 ph_port_state_t state)
{
    ...
    size_t open_count = (state == PH_PORT_OPEN)
                            ? (atomic_fetch_add(&progress->open_count, 1) + 1)
    ...
    if (nyx_logger_verbose > 0) {
        const char *state_str = (state == PH_PORT_OPEN) ? "open" :
                                (state == PH_PORT_CLOSED) ? "closed" : "filtered";
    ...
    if (nyx_term_is_interactive()) {
        nyx_term_statusf("[*] Portscan %zu/%zu ports (open: %zu) current=%u/tcp",
                         completed, progress->total, open_count,
                         (unsigned)progress->last_port);
    } else if (completed == 1 || completed == progress->total ||
               (completed % 64) == 0) {
        nyx_log(NYX_LOG_INFO,
                "Portscan progress: %zu/%zu ports scanned (open: %zu)",
                completed, progress->total, open_count);
    }
...
static void *scan_worker(void *arg)
{
    ...
    for (size_t i = 0; i < w->count; i++) {
        ...
        if (w->mode == PH_PORTSCAN_TCP_SYN) {
            w->results[idx].state = syn_scan_port(
                w->src_ip, w->target_nbo, port,
                send_fd, recv_fd, w->timeout_ms);
        } else {
            w->results[idx].state = connect_scan_port(
                w->target, port, w->timeout_ms);
        }

        scan_progress_update(w->progress, port, w->results[idx].state);
    }

    if (send_fd >= 0) nyx_socket_close(send_fd);
    if (recv_fd >= 0) nyx_socket_close(recv_fd);
    return NULL;
}
                     ...
                     ph_portscan_result_t **result)
{
    ...
    if (!config->target[0]) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_INVALID_TARGET,
                         NYX_ERROR_SEV_ERROR, "Empty target in configuration",
                         "Use -t to specify a target IP address");
    ...
    if (mode == PH_PORTSCAN_TCP_SYN && geteuid() != 0) {
        nyx_log(NYX_LOG_WARN,
                "SYN scan requires root -- falling back to TCP Connect");
    ...
    if (ret != 0) {
        free(port_list);
        NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_MEMORY,
                         NYX_ERROR_SEV_ERROR,
                         "Memory allocation failed for port list",
                         "Reduce port range");
        return ret;
    }
    if (port_count == 0) {
        free(port_list);
        NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_INVALID_PARAM,
                         NYX_ERROR_SEV_ERROR, "No ports to scan",
                         "Specify a port range (-p) or top-ports count (-P)");
        return PH_PORTSCAN_ERR_INVALID_PARAM;
    }
    ...
    if (mode == PH_PORTSCAN_TCP_SYN) {
        ...
        if (src_ip == 0) {
            nyx_log(NYX_LOG_WARN,
                    "Cannot resolve local IP -- falling back to TCP Connect");
            mode = PH_PORTSCAN_TCP_CONNECT;
        } else if (!can_use_syn_mode(timeout)) {
            nyx_log(NYX_LOG_WARN,
                    "Raw TCP sockets unavailable -- falling back to TCP Connect");
            mode = PH_PORTSCAN_TCP_CONNECT;
        }
    }
    ...
    if (!res->ports) {
        ...
        free(port_list);
        NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_MEMORY,
                         NYX_ERROR_SEV_CRITICAL, "Failed to allocate port array",
                         "System may be low on memory");
        return PH_PORTSCAN_ERR_MEMORY;
    }
    ...
    nyx_log(NYX_LOG_INFO,
            "Scanning %zu ports on %s [%s mode, %d threads, %d ms timeout]",
    ...
    for (int i = 0; i < nthreads; i++) {
        if (pthread_create(&threads[i], NULL, scan_worker, &work[i]) != 0) {
            ...
            free(res);
            free(port_list);
            NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_THREAD,
                             NYX_ERROR_SEV_ERROR,
    ...
    nyx_log(NYX_LOG_SUCCESS,
            "Scan complete: %zu open / %zu scanned (%.1f ms)",
            res->open_count, res->scanned_count, res->elapsed_ms);
                              ...
                              int open_only)
{
    ...
    printf("\nScanned %zu ports on %s in %.2fs -- %zu open\n",
           result->scanned_count, result->target,
           result->elapsed_ms / 1000.0, result->open_count);
}
