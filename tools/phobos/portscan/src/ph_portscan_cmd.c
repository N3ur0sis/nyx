...
int ph_portscan_cmd_invoke(const nyx_json_t *params, nyx_output_ctx_t *out)
{
    ...
    if (!target || !target[0]) {
        NYX_ERROR_SET_EX(NYX_DOMAIN_PORTSCAN, PH_PORTSCAN_ERR_INVALID_TARGET, NYX_ERROR_SEV_ERROR,
                         "No target IP specified", "Provide 't' or 'target' parameter");
    ...
    if (!port_str)
        port_str = json_str(params, "ports");
    ...
    if (port_str) {
        ...
        cfg.top_ports = (int)json_int(params, "P",
                                      json_int(params, "top-ports", PH_PORTSCAN_DEFAULT_TOP_PORTS));
    }

    const char *mode_str = json_str(params, "m");
    if (!mode_str)
        mode_str = json_str(params, "mode");
    if (mode_str && (strcmp(mode_str, "syn") == 0 || strcmp(mode_str, "SYN") == 0))
        cfg.mode = PH_PORTSCAN_TCP_SYN;
    else if (mode_str)
        cfg.mode = PH_PORTSCAN_TCP_CONNECT;
    else
        cfg.mode = (geteuid() == 0) ? PH_PORTSCAN_TCP_SYN : PH_PORTSCAN_TCP_CONNECT;

    cfg.threads =
        (int)json_int(params, "T", json_int(params, "threads", PH_PORTSCAN_DEFAULT_THREADS));
    cfg.timeout_ms =
        (int)json_int(params, "w", json_int(params, "timeout", PH_PORTSCAN_DEFAULT_TIMEOUT));

    int open_only = json_bool(params, "o", json_bool(params, "open-only", 0));
    ...
    nyx_json_set(jcfg, "mode",
                 nyx_json_string(cfg.mode == PH_PORTSCAN_TCP_SYN ? "syn" : "connect"));
    ...
    nyx_json_set(results, "scan_mode",
                 nyx_json_string(result->actual_mode == PH_PORTSCAN_TCP_SYN ? "syn" : "connect"));
...
static const nyx_repl_flag_t scan_flags[] = {
    {"-t", NYX_COMPL_NONE},        {"--target", NYX_COMPL_NONE},    {"-p", NYX_COMPL_NONE},
    {"--ports", NYX_COMPL_NONE},   {"-P", NYX_COMPL_NONE},          {"--top-ports", NYX_COMPL_NONE},
    {"-m", NYX_COMPL_NONE},        {"--mode", NYX_COMPL_NONE},      {"-T", NYX_COMPL_NONE},
    {"--threads", NYX_COMPL_NONE}, {"-w", NYX_COMPL_NONE},          {"--timeout", NYX_COMPL_NONE},
    {"-o", NYX_COMPL_NONE},        {"--open-only", NYX_COMPL_NONE},
};
