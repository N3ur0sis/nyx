/**
 * ph_iface.c - Network Interface Utility Library
...
static int map_error(int err, int default_err) {
    switch (err) {
        case EACCES:
        case EPERM:
            return PH_IFACE_ERR_PERM;
        case ENOENT:
            return PH_IFACE_ERR_NOTFOUND;
        case EBUSY:
            return PH_IFACE_ERR_BUSY;
        case ENOMEM:
        case EMFILE:
        case ENFILE:
            return PH_IFACE_ERR_SOCKET;
        case EIO:
            return PH_IFACE_ERR_IO;
        default:
            return default_err;
    }
}
...
int ph_iface_get_mac(const char *iface, char *buffer, size_t len) {
    if (!iface || !buffer || len < PH_IFACE_MAX_MAC_LEN) {
        return PH_IFACE_ERR_PARAM;
    }
    ...
    if (!fp) {
        ...
        return map_error(err, PH_IFACE_ERR_IO);
    }
...
int ph_iface_get_ipv4(const char *iface, char *buffer, size_t len) {
    ...
    if (sock < 0) {
        ...
        return map_error(err, PH_IFACE_ERR_SOCKET);
    }
...
int ph_iface_list(char interfaces[][IFNAMSIZ], size_t max_count, size_t *count) {
    ...
    *count = 0;
    ...
    while ((entry = readdir(dir)) != NULL && *count < max_count) {
        ...
        if (ph_iface_is_valid(entry->d_name)) {
...
int ph_iface_get_mac_by_ip(const char *iface, const char *ip_addr, char *mac_buffer, size_t len) {
    if (!iface || !ip_addr || !mac_buffer || len < PH_IFACE_MAX_MAC_LEN) {
        return PH_IFACE_ERR_PARAM;
    }
    ...
    if (ioctl(sock, SIOCGARP, &arpreq) < 0) {
        ...
        return map_error(err, PH_IFACE_ERR_NOTFOUND);
    }
...
int ph_iface_print_details(void) {
    ...
    result = ph_iface_list(interfaces, 32, &count);
    ...
    for (size_t i = 0; i < count; i++) {
        char mac[PH_IFACE_MAX_MAC_LEN] = {0};
        char ip[INET_ADDRSTRLEN] = {"N/A"};
        char netmask[INET_ADDRSTRLEN] = {"N/A"};
        int is_up = ph_iface_is_up(interfaces[i]);
        
        ph_iface_get_mac(interfaces[i], mac, sizeof(mac));
        ph_iface_get_ipv4(interfaces[i], ip, sizeof(ip));
        ph_iface_get_netmask(interfaces[i], netmask, sizeof(netmask));
...
int ph_iface_get_default_gateway(char *buffer, size_t len) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        fields = sscanf(line, "%s %lx %lx", iface, &dest, &gateway);
        
        if (fields == 3 && dest == 0) { // Default route (0.0.0.0)
    ...
    fclose(fp);
    return found ? PH_IFACE_SUCCESS : PH_IFACE_ERR_NOTFOUND;
}
...
int ph_iface_get_gateway_ip(char *buffer, size_t len) {
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (fields == 3 && dest == 0) { // Default route (0.0.0.0)
            ...
            if (!inet_ntop(AF_INET, &gw_addr, buffer, len)) {
                int err = errno;
                fclose(fp);
...
int ph_iface_add_ipv4(const char *iface, const char *ip_addr, int prefix_len) {
    if (!ph_iface_is_valid(iface) || !ip_addr) {
        return PH_IFACE_ERR_PARAM;
    }
    ...
    sin->sin_family = AF_INET;
    sin->sin_addr.s_addr = prefix_len ? htonl(~((1 << (32 - prefix_len)) - 1)) : 0;
